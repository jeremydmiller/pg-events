...
var loadExternalModule = function(name){
	var path = require.resolve(name);

	return loadModule(path, name);
}

var loadInternalModule = function(path){
	return loadModule(__dirname + path, Path.basename(path, '.js'));
}
...
exports.seedAll = function(options){
	...
	var operations = [
		...
		loadFile('async_projection_handling.sql'),
		createBuffer,
		logTables,
		loadInternalModule('/server/eventstore.js'),
		loadInternalModule('/server/postgres-store.js'),
		loadInternalModule('/projection-plan.js'),
		loadInternalModule('/projection-library.js'),
		loadInternalModule('/projections.js'),
		loadInternalModule('/stream-aggregator.js'),
		loadInternalModule('/event-projector.js'),
		loadInternalModule('/aggregate-projector.js'),
		loadInternalModule('/snapshot-aggregator.js'),
		loadInternalModule('/store-base.js'),
...
exports.rebuildProjections = function(options){
	...
	var loader = Promise.coroutine(function*(){
		...
		return db.transaction(function*(t){
			yield t.query('delete from pge_projection_definitions');
			...
			for (var i = 0; i < files.length; i++){
				...
				yield t.query('insert into pge_projection_definitions (name, definition) values (?, ?)', [name, text]);
			}
