...
function Projector(){
	...
	self.projectStream = function(definition){
		...
		var aggregator = new StreamAggregator(definition.name, definition);
		self.byStream.add(definition.stream, aggregator);
		self.projections[definition.name] = aggregator;

		if (definition.hasOwnProperty('async')){
			aggregator.async = definition.async;
		}
	...
	self.projectAcrossStreams = function(name){
		return {
			by: function(definition){
				var aggregator = new AggregateProjector(name, definition);
	...
	self.projectEvent = function(config){
		...
		var eventProjector = new EventProjector(config, config.transform);
		if (config.hasOwnProperty('async')){
			eventProjector.async = config.async;
		}
