...
ProjectionLibrary.prototype.compile = function(){
	...
	for (var i = 0; i < events.length; i++){
		...
		this.replayPlans[e] = new ProjectionPlan(e);
	}
...
ProjectionLibrary.prototype.replayPlanFor = function(eventType){
	return this.replayPlans[eventType] || new ProjectionPlan(eventType);
}
...
ProjectionLibrary.prototype.addToPlan = function(projection){
	for (var i = 0; i < projection.events.length; i++){
		...
		if (projection.mode == 'sync'){
			...
			this.replayPlans[e].add(projection);
		}
