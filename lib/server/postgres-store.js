function ProjectionStore(type){
	...
	this.queueSql = 'select pge_*_append_rolling_buffer($1, $2)'.replace('*', type);
...
ProjectionStore.prototype.queue = function(eventId, streamId){
	plv8.execute(this.queueSql, [eventId, streamId]);
}
...
var storage = {
	...
	queueProjectionEvent: function(name, id, stream){
		this.forViewType(name).queue(id, stream);
	},
