...
module.exports = function(persistor, projector, aggregates){
	return {
		findStream: function(message, events){
			...
			if (existing == null){
				var streamType = message.type || aggregates.streamTypeForEvent(events[0].$type);
				if (streamType == null){
		...
		store: function(message){
			...
			if (aggregates.requiresSnapshotUpdate(stream.type, next, stream.snapshotVersion)){
				aggregates.rebuild(persistor, stream.id);
			}
